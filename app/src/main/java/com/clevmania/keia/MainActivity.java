...
public class MainActivity extends AppCompatActivity {
    ...
    private String STT_username, STT_password, TTS_username, TTS_password;
...
//        btnRecord= findViewById(R.id.btn_record);
        ...
        symptomsAdapter = new SymptomsAdapter(messageArrayList);
        ...
        recyclerView.addOnItemTouchListener(new TouchListener(getApplicationContext(), recyclerView, new ClickListener() {
            ...
            public void onClick(View view, final int position) {
                Thread thread = new Thread(new Runnable() {
                    public void run() {
                        ChatMessageModel audioMessage;
                        try {

                            audioMessage =(ChatMessageModel) messageArrayList.get(position);
                            streamPlayer = new StreamPlayer();
                            if(audioMessage != null && !audioMessage.getMsg().isEmpty())
                                //Change the Voice format and choose from the available choices
                                streamPlayer.playStream(textToSpeech.synthesize(audioMessage.getMsg(), Voice.EN_LISA).execute());
                            else
...
//        });
    ...
    private void sendMessage() {
        ...
        if(!this.initialRequest) {
            ChatMessageModel inputMessage = new ChatMessageModel();
            inputMessage.setMsg(inputmessage);
        ...
        Thread thread = new Thread(new Runnable(){
            public void run() {
                try {
                    ...
                    final ChatMessageModel outMessage=new ChatMessageModel();
                    if(response!=null)
                    {
                        if(response.getOutput()!=null && response.getOutput().containsKey("text"))
                        {
                            ...
                            if(null !=responseList && responseList.size()>0){
                                outMessage.setMsg((String)responseList.get(0));
                                outMessage.setId("2");
                            }
                        ...
                        runOnUiThread(new Runnable() {
                            public void run() {
                                symptomsAdapter.notifyDataSetChanged();
                                if (symptomsAdapter.getItemCount() > 1) {
                                    recyclerView.getLayoutManager().smoothScrollToPosition(recyclerView, null, symptomsAdapter.getItemCount()-1);
